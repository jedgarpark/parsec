...
try:
    splash_bitmap = displayio.OnDiskBitmap("spyce_splash.bmp")  # a 400x240 4-bit BMP file works well here
    ...
    print("Showing splash screen for 5 seconds...")
    time.sleep(10)
    print("Starting game!")
...
explosion_lines = []
explosion_timer = 0
explosion_duration = 1.0  # Show explosion for 1 second

# Create the enemy as a rectangle
enemy = Rect(enemy_x, enemy_y, enemy_width, enemy_height, fill=black)
screen_group.append(enemy)
print(
    f"Enemy rectangle created at position ({enemy_x}, {enemy_y}) with size {enemy_width}x{enemy_height}"
)
print(
    f"Enemy will move in steps of {enemy_step} pixels every {enemy_move_delay} seconds"
)
...
def create_explosion(center_x, center_y):
    ...
    explosion_center_x = center_x + enemy_width // 2
    explosion_center_y = center_y + enemy_height // 2

    # Create 10 lines radiating outward
    for i in range(10):
        angle = (i * 36) * (math.pi / 180)  # 36 degrees apart (360/10)
        line_length = 30
...
def respawn_enemy():
    """Respawn the enemy at starting position"""
    global enemy_alive, enemy_x, enemy_y, enemy_direction
    enemy_alive = True
    enemy_x = 50
    enemy_y = 50
    enemy_direction = 1
...
while True:
    ...
    if projectile is not None:
        ...
        # Check for collision with enemy (only if enemy is alive)
        collision_occurred = False
        if enemy_alive:
            if (
                projectile.x + projectile_radius
                >= enemy_x  # Projectile right edge >= enemy left edge
                and projectile.x - projectile_radius
                <= enemy_x + enemy_width  # Projectile left edge <= enemy right edge
                and projectile.y + projectile_radius
                >= enemy_y  # Projectile bottom edge >= enemy top edge
                and projectile.y - projectile_radius <= enemy_y + enemy_height
            ):  # Projectile top edge <= enemy bottom edge
                # Enemy hit! Create explosion
                screen_group.remove(enemy)  # Remove enemy from display
                enemy_alive = False
                explosion_timer = 0
        ...
        # Remove projectile if it goes off screen (only if it wasn't removed by collision)
    ...
    if not enemy_alive and explosion_lines:
        explosion_timer += 0.01
        if explosion_timer >= explosion_duration:
            clear_explosion()
            respawn_enemy()

    # Update enemy movement (only if alive)
    if enemy_alive:
